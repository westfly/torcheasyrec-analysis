...
> 内置损失函数（BinaryFocalLoss、JRCLoss、ParetoEfficientMultiTaskLoss）的详细解析见 [12-损失函数](11-loss)。
...
> 自定义指标（GroupedAUC、XAUC、NormalizedEntropy 等）详细解析见 [13-评估指标](12-metrics)。
...
> 优化器类型列表和 LR 调度器（ExponentialDecayLR、CosineAnnealingLR 等）详细配置见 [14-优化器与学习率调度器](13-optimizer)。
